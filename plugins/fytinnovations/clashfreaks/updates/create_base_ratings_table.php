...
class CreateBaseRatingsTable extends Migration
{
    public function up()
    {
        Schema::create('fytinnovations_clashfreaks_base_ratings', function (Blueprint $table) {
            ...
            $table->timestamps();
            $table->foreign('user_id', 'base_ratings_user_id')->references('id')->on('users');
            $table->foreign('base_design_id', 'base_ratings_base_design_id')->references('id')->on('fytinnovations_clashfreaks_base_designs');
        });
    }
    ...
    public function down()
    {
        Schema::table('fytinnovations_clashfreaks_base_ratings', function ($table) {
            $table->dropForeign('base_ratings_user_id', 'base_ratings_base_design_id');
        });
